...
public class AllCardMainActivity extends OcrBaseActivity implements IBaseReturnMessage {
    ...
    public void jumpIntelligenceScanActivity() {
        CardOcrRecogConfigure.getInstance()
                ...
                //设置图片的存储路径(注：默认路径为:context.getFilesDir().getPath()+ "/wtimage/")
...
//            importPicToRecog(selectPath);
    ...
    private void CameraScanSuccess(Bundle bundle) {
        ...
        try {
            ...
            intent.putExtra("HeadJpgPath", picPath[2]);
            intent.putExtra("mainId", getSharedPreferencesStoreMainId());
     ...
     * 扫描识别失败，界面的跳转
     *
     * @param error -10601 开发码错误,把该文件中的{@link Devcode}中的devcode替换
     *              -10602 applicationId错误，把build.gradle文件中的 applicationId修改为授权文件中绑定的信息
     *              -10603 授权到期，请从新申请授权
     *              -10604 识别引擎版本号和授权文件不匹配
