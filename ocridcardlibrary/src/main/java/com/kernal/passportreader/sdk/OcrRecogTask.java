...
public class OcrRecogTask implements Runnable {
    ...
    private boolean isRunImport = false;
    ...
    public OcrRecogTask(Context context, OcrIdCardRecogParams ocrIdCardRecogParams) {
        ...
        this.isRunImport = Devcode.isRunImport;
    }
...
//                    Log.e("TAG", "run: "+LoadBufferImage+"*-*" +ConfirmSideSuccess+"*-*"+CheckPicIsClear+"*-*"+detectLightspot);
                    //图片加载成功&&检测边线成功&&检测清晰度成功&&没有光斑，进行识别
                    if (LoadBufferImage == 0 && ConfirmSideSuccess >=0
                            && CheckPicIsClear == 0 && detectLightspot != 0) {
                        if (isRunImport || nMainIDX == 34 || nMainIDX == 35 || nMainIDX == 36 || nMainIDX == 37) {
                            bitmap = nv21ToBitmap(data, ocrIdCardRecogParams.preWidth, ocrIdCardRecogParams.preHeight);
                        }
                        picPathString = CardOcrRecogConfigure.getInstance().savePath.getFullPicPath();
                        cutPicPath = CardOcrRecogConfigure.getInstance().savePath.getCropPicPath();
                    ...
                    if (LoadBufferImage == 0) {
                        ...
                        if (ConfirmSideSuccess == 1034
                            ...
                            RecogService.nMainID = ConfirmSideSuccess;
                        }
                    }
    ...
    public void getRecogResult() {
        ...
        rpm.lpHeadFileName =HeadJpgPath ;//Save portrait of identity document
        rpm.cutSavePath=cutPicPath;
        rpm.isCut = false;
        try {
            resultMessage = ocrIdCardRecogParams.recogBinder.getRecogResult(rpm);
//            Log.e("TAG", "getRecogResult: "+resultMessage.ReturnAuthority+"*-*" +resultMessage.ReturnInitIDCard+"*-*"+resultMessage.ReturnLoadImageToMemory+"*-*"+resultMessage.ReturnRecogIDCard);
